...
It can expose the app via a classic **Ingress**, an OpenShift **Route**, or a
...
On OpenShift/OKD, a native `Route` gives you direct control over TLS termination
and the HAProxy router annotations. Leave `route.host` empty and the cluster
generates `<release>-todo-<namespace>.<apps-domain>` for you:
...
With your own hostname and edge termination (the router terminates TLS and talks
plain HTTP to the pod — what this app expects):

```sh
helm install todo oci://ghcr.io/inqbeo/charts/todo \
  --set route.enabled=true \
  --set route.host=todo.apps.example.com \
  --set route.tls.termination=edge \
  --set route.tls.insecureEdgeTerminationPolicy=Redirect
```

Set `route.tls.enabled=false` for a plain HTTP Route. A custom certificate goes
into `route.tls.certificate` / `route.tls.key` (PEM); leave them empty to use the
router's default wildcard certificate.

### 6. Expose via Gateway API HTTPRoute
...
| `route.host` | `""` | Route hostname; empty → generated by OpenShift. |
| `route.path` | `""` | Optional path prefix (not valid for `passthrough`). |
| `route.annotations` | `{}` | Router annotations (`haproxy.router.openshift.io/...`). |
| `route.wildcardPolicy` | `None` | `None` \| `Subdomain`. |
| `route.tls.enabled` | `true` | `false` → plain HTTP Route. |
| `route.tls.termination` | `edge` | `edge` \| `passthrough` \| `reencrypt`. |
| `route.tls.insecureEdgeTerminationPolicy` | `Redirect` | `None` \| `Allow` \| `Redirect`. |
| `route.tls.certificate` / `key` / `caCertificate` / `destinationCACertificate` | `""` | Custom PEM material; empty → router default certificate. |
